package org.towerhawk.monitor.check.threshold.builder;
...
public class SimpleNumericBuilder {
	...
	public SimpleNumericThreshold build() {
		if (warning != null && critical != null) {
			return new SimpleNumericThreshold(warning, critical, addContext, setMessage);
		} else {
			return new SimpleNumericThreshold(warnLower, warnUpper, critLower, critUpper, between, precision, addContext, setMessage);
		}
	}
}